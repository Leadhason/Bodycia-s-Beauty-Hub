  ...
  return (
    <section id="team" className="py-24 px-6 md:px-12">
      <div className="max-w-7xl mx-auto">
        <p className="text-mauve text-sm font-medium text-center tracking-wide mb-6">Bodycia's Team</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {TEAM_DATA.map((member, idx) => {
            const isInstagramLink = member.role === "Visit Instagram";
            const MemberCard = isInstagramLink ? "a" : "div";
            const linkProps = isInstagramLink ? {
              href: "https://instagram.com/bodyciasbeauty",
              target: "_blank",
              rel: "noopener noreferrer"
            } : {};
            
            return (
              <MemberCard key={idx} className="group cursor-pointer" {...linkProps}>
                <div className="overflow-hidden rounded-xl mb-6 h-[500px] relative">
                  ...
                  <div className="absolute bottom-6 left-6 right-6 bg-cream/95 backdrop-blur-sm p-6 rounded-lg text-center shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-transform">
                    ...
                    <p className={`font-medium text-xs tracking-widest uppercase ${isInstagramLink ? 'text-mauve hover:underline' : 'text-mauve'}`}>
                      {member.role}
                    </p>
                  </div>
                </div>
